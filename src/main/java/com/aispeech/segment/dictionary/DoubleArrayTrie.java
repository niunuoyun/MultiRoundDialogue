...
public class DoubleArrayTrie
{
    ...
    private int fetch(Node parent, List<Node> siblings)
    {
        ...
        {
            if( (length != null ? length[i] : key.get(i).getValue().length()) < parent.depth )
                continue;

            String tmp = key.get(i).getValue();
    ...
    public int build(List<Word> key)
    {
        return build(key, null, null, key.size());
    }

    public int build(List<Word> _key, int _length[], int _value[], int _keySize)
    {
    ...
    public static void main(String[] args) {
        DoubleArrayTrie dictionary = new DoubleArrayTrie();

        List<Word> words = new ArrayList<>();
        Word word = new Word("百度","n",200);
        Word word1 = new Word("是","adj",200);
        Word word2 = new Word("adj","干",200);
        Word word3 = new Word("v,ving","搜索",200);
        words.add(word);
        words.add(word1);
        words.add(word2);
        words.add(word3);
        ...
        System.out.println("查找百度：" + dictionary.commonPrefixSearch("百度"));
    }
