...
public class Utils {
  ...
  /*  public static void seg(File input, File output, boolean removeStopWords, SegmentationEnum segmentationEnum) throws Exception{
        seg(input, output, removeStopWords, segmentationEnum, null);
    }*/

    /*public static void seg(File input, File output, boolean removeStopWords, SegmentationEnum segmentationEnum, FileSegmentationCallback fileSegmentationCallback) throws Exception{
        LOGGER.info("开始对文件进行分词："+input.toString());
        Segmentation segmentation = SegmentationFactory.getSegmentation(segmentationEnum);
        ...
        try(BufferedReader reader = new BufferedReader(new InputStreamReader(new FileInputStream(input),"utf-8"));
            ...
            while((line = reader.readLine()) != null){
                ...
                List<Word> words = new ArrayList<>(); // segmentation.segment(line);
                if(removeStopWords){
                    //停用词过滤
                    StopWordResourceLoader.filterStopWords(words);
                }
                if(words == null){
                    continue;
                }
